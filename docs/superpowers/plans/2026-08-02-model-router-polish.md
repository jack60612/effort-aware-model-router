...
2. Implement the dialog flow using only public `ctx.ui.select`, `ctx.ui.input`, `ctx.ui.confirm`, and `ctx.hasUI`: scope; enabled; minimum prompt length; cooldown; classifier selectors; each threshold's ordered candidates from `ctx.models.list()` plus custom selector input; optional model-specific profile with default and `xhigh`/`max` overrides; final summary/confirmation.
